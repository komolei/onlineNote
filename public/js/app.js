...
var Toast = function () {
  ...
  _createClass(Toast, [{
    ...
    value: function create() {
      var container = '<span>' + this.message + '</span>';
...
function waterFall() {
  var $img = (0, _JQ2.default)('span');
  // console.log('span22', $img);
  var $item = (0, _JQ2.default)('.note');
  var arrimg = []; // 存放元素的高度
  var colsum = parseInt((0, _JQ2.default)('.waterfall.ct').outerWidth() / $item.outerWidth());
  console.log('colsum', colsum);
  for (var i = 0; i < colsum; i++) {
    arrimg[i] = 0;
  }
  $item.each(function () {
    var img = (0, _JQ2.default)(this).children().find('span');
    img.css({
      height: (0, _JQ2.default)(this).height()
    });
    var min = Math.min.apply(null, arrimg);
    var minIndex = arrimg.indexOf(min);
    (0, _JQ2.default)(this).css({
      top: arrimg[minIndex] + (0, _JQ2.default)('.addNote').outerHeight(), // 这些要用在定位的元素上的。妈蛋，现在才想起来。
      left: minIndex * $item.outerWidth(true)
    });
    arrimg[minIndex] += (0, _JQ2.default)(this).outerHeight(true); // 给添加好的元素记录高度，为下次的比较而准备。
  });
}
...
var ct = document.querySelector('.waterfall.ct');
...
fetch('/api/notes', {
  ...
  return r.notes.forEach(function (item) {
    return new Note({ container: '', content: item.context });
  });
});
// };
...
var Note = function () {
  ...
  _createClass(Note, [{
    ...
    value: function bindEvent() {
      var noteId = document.getElementById(this.id);
      Event.on('delete', function (val) {
        console.log('val:', val);
        if (val === noteId.id) {
    ...
    value: function moveEvent() {
      ...
      var noteCt1 = document.getElementById(this.id);
      var noteCt = document.getElementsByClassName('pos')[this.id - 1];
      console.log('noteCt', noteCt);
      noteCt.onmousedown = function () {
        noteCt.style.cursor = 'move';
        var result = _this.getMousePos(event);
        console.log('onmousedown', result, _this);
      };
      noteCt.onmousemove = function () {
        var result = _this.getMousePos(event);
        var oW = parseInt(noteCt.offsetWidth / 2);
        var oH = parseInt(noteCt.offsetHeight / 2);
        var oW1 = parseInt(noteCt1.offsetWidth / 2);
        var oH1 = parseInt(noteCt1.offsetHeight / 2);
        var oL = noteCt.offsetLeft;
        var oT = noteCt.offsetTop;
    ...
    value: function edit() {
      ...
      var spanCt = document.querySelectorAll('.noteCt>span')[this.id - 1];
      var inputValue = document.querySelectorAll('input')[this.id - 1];
